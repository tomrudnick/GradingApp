...
class CourseListViewModel : ObservableObject {
    ...
    init() {
        courseViewModels = createDummyData()
    }
    ...
    private func parseCSVGradeFile(course: Course, csvData: String, gradeType: GradeType) {
        if let courseViewModel = courseViewModels.first(where: {$0.course.name == course.name}) {
            let csv = try! CSVReader(string: csvData, hasHeaderRow: true)
            let gradeHeaderRow = csv.headerRow!
            while let row = csv.next() {
                let firstName = row[1]
                let lastName = row[2]
                let student = courseViewModel.course.findStudent(firstName: firstName, lastName: lastName)
    ...
    private func createDummyData() -> [CourseViewModel] {
        let courses = createDummyCourses()
        if let courseViewModel = courses.first(where: {$0.course.name == "Mathe 10FLS" }) {
            parseCSVGradeFile(course: courseViewModel.course, csvData: csvNotenMuenlich10FLS, gradeType: .oral)
        }
        return courses
    }
    
    private func createDummyCourses() -> [CourseViewModel]{
        return [CourseViewModel(course: parseCSVCourseFile(csvData: csvMathe7F)),
                CourseViewModel(course: parseCSVCourseFile(csvData: csvMathe6C)),
                CourseViewModel(course: parseCSVCourseFile(csvData: csvMathe10FLS))
                ]
    }
    

    func addCourse(courseTitle: String) {
        courseViewModels.append(CourseViewModel(course: Course(name: courseTitle)))
    }
